from hcp_builder.dataset import fetch_behavioral_data, fetch_subject_list, \
    fetch_hcp, _convert_to_s3_target
...
def test_fetch_hcp():
    ...
    assert res.shape[0] >= 970
    res = fetch_hcp(n_subjects=1)
    res = fetch_hcp(n_subjects=1, data_type='rest')
    res = fetch_hcp(on_disk=True, data_type='task')
    res = fetch_hcp(on_disk=True, data_type='task', tasks='EMOTION')
    res = fetch_hcp(on_disk=True, data_type='rest')
    res = fetch_hcp(on_disk=False, data_type='rest')